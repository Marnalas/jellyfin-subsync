...
services:
  ...
  jellyfin-subsync:
    ...
    environment:
      ...
      # Nothing is added here automatically. On a specific job, the sidecar
      # may still decide on its own to use --vad webrtc, when the plugin
      # reports that Jellyfin's subtitle metadata says the video's only
      # embedded subtitle track is a forced-only stub - an explicit --vad
      # set here always overrides that. See docs/CONFIGURATION.md for both
      # of these.
